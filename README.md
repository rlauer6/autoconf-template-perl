...
situations where you do want to perform this kind of check.  Building
RPMs for example, that will resolve dependencies in the target
environment is one such case.  Your build system may not, and does not
need the Perl modules installed there. To deal with this situation,
there are two options you should consider.

Use the `--disable-perldeps` option to `configure` to disable
the checking of Perl modules and the `--enable-rpm-build-mode` which
will turn off syntax checking of Perl scripts and modules.
Substitution of `automake` variables will still occur.


## Perl Module Dependencies

As discussed in previous sections, `autoconf-template-perl` will
examine your source files and identify Perl module dependencies. Those
dependencies are then used to create an `m4` macro used in the
`configure` script to check your system for those dependencies. When
you manually add new Perl scripts or modules you can run
`autoconf-template-perl --refresh` to update the list of
dependencies. If you use the `--create-stub` option, the refresh is
run automatically for you. Refreshing your project does more than just
update depdencies, it also updates the list of assets in the
`Makefile`s that are built or installed.

You may not want `autoconf-template-perl` to alter your the list of
requirements if you have previously edited the requirement files. In
...
Use the `--pod-to-readme` option for creating a `README.md` for your
Perl modules.  The `README.md` files are __not__ automatically created
during a build. Execute `make docs` in the `src/main/perl/lib`
directory to create your `README.md` files. Each files will be created
in a sub-directory whose name is the Perl module path.  Adding these
files to your `git` repo gives you a convenient way to look at the
